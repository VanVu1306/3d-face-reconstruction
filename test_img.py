...
from model import VggEncoder, ResNetEncoder
import time
...
model = ResNetEncoder()
# model = torch.nn.DataParallel(model) # bỏ .cuda()
ckpt = torch.load('data/resnet50_mvfnet.pth', map_location=torch.device('cpu'))
state = ckpt["model_state_dict"]   # extract the real weights

model.load_state_dict(state)
# model.load_state_dict(ckpt)
#print model
input_tensor = torch.cat([imgA, imgB, imgC], 0).view(1, 9, 224, 224) # bỏ .cuda()
start = time.time()
preds = model(input_tensor)
print(time.time() -start)
faces3d = tools.preds_to_shape(preds[0].detach().cpu().numpy())
tools.write_ply(os.path.join(options.save_dir, 'shape_test.ply'), faces3d[0], faces3d[1])
